class ApplicationController < ActionController::API
  ...
  def retrievehome
    ...
    # render json: @retrievehome, status: :ok
  end
  # def retrieve_direct_message(second_user)
  #   @m_user = MUser.find_by(id: second_user)
  #   TDirectMessage.where(send_user_id: @current_user, receive_user_id: second_user, read_status: 0).update_all(read_status: 1)
  #   TDirectThread.joins("INNER JOIN t_direct_messages ON t_direct_messages.id = t_direct_threads.t_direct_message_id").where(
  #     "(t_direct_messages.receive_user_id = ? and t_direct_messages.send_user_id = ? ) || (t_direct_messages.receive_user_id = ? and t_direct_messages.send_user_id = ? )", second_user,  @current_user, @current_user, second_user
  #   ).where.not(m_user_id: second_user, read_status: 1).update_all(read_status: 1)
  ...
  #                                         second_user,  @current_user,  @current_user, second_user).order(created_at: :desc)
  ...
  #                                           .where("(t_direct_messages.receive_user_id = ? and t_direct_messages.send_user_id = ? ) 
  #                                           || (t_direct_messages.receive_user_id = ? and t_direct_messages.send_user_id = ? )", 
  #                                           second_user,  @current_user,  @current_user, second_user)
  ...
  def retrieve_direct_message
    @m_user = MUser.find_by(id: @current_user)
    TDirectMessage.where(send_user_id: params[:id], receive_user_id: @m_user.id, read_status: 0).update_all(read_status: 1)
    TDirectThread.joins("INNER JOIN t_direct_messages ON t_direct_messages.id = t_direct_threads.t_direct_message_id").where(
      "(t_direct_messages.receive_user_id = ? and t_direct_messages.send_user_id = ? ) || (t_direct_messages.receive_user_id = ? and t_direct_messages.send_user_id = ? )", @m_user.id,  params[:id],  params[:id], @m_user.id
    ).where.not(m_user_id: @m_user.id, read_status: 1).update_all(read_status: 1)
    ...
    #   temp_direct_star_msgids: @temp_direct_star_msgids,
  ...
  def retrieve_direct_thread(direct_message_id)
    ...
    TDirectThread.where.not(m_user_id: @current_user, read_status: 1).update_all(read_status: 1)
